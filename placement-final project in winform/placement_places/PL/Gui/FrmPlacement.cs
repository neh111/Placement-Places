...
namespace placement_places.Gui
{
    public partial class FrmPlacement : Form
    {
        ...
        public FrmPlacement()
        {
            ...
            //var students = DB.students_tbl.Where(x => x.id_class ==);
        }
        //public FrmPlacement(ClassXmlBll c)
        //{
        //    InitializeComponent();
        //    this.ClassToPlacement = c;
        //    DrawTables();
        //}
        public void DrawTables()
        {
            ...
            for (int i = 0; i < ClassToPlacementBLL.NumCoulmns; i++)
            {
                for (int j = 0; j < ClassToPlacementBLL.NumLines; j++)
                {
                    ...
                    table.Coulmn = i;
                    table.Line = j;
                    //table.TableConstraints = ClassToPlacement.MatConstraints[j, i];
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            students = DB.students_tbl.Where(x => x.class_tbl.id_class == this.ClassToPlacement.id_class).ToList();
        ...
        private void btnSavePlacement_Click(object sender, EventArgs e)
        {
            foreach (var item in panel1.Controls)
            {
                if (item is DeskUserControl)
                {
                    ...
                    DB.SaveChanges();
                    //this.ClassToPlacementBLL.SaveStudentPlacementInSqlBLL(d.StudentA, d.Line, d.Coulmn);
                    //this.ClassToPlacementBLL.SaveStudentPlacementInSqlBLL( d.StudentB, d.Line, d.Coulmn);
                }
            }
        ...
        {
            students.Remove(s);
            lstStudentsToPLace.DataSource = students.ToList();
        }

        private void lstStudentsToPLace_SelectedIndexChanged(object sender, EventArgs e)
        {
